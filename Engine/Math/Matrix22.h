...
namespace nc {
	struct Matrix22 {
		float m[2][2]; // [coloumn][row]
		Matrix22();
		Matrix22(const Vector2& column1, Vector2& column2);
	...
	inline Matrix22::Matrix22(const Vector2& column1, Vector2& column2) {
		m[0][0] = column1[0]; m[1][0];
		m[0][1] = column2[1]; m[1][1];
	}
